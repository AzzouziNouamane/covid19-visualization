...
const computeOpacities = (regionsNewCases, minNewCases, maxNewCases) => {
  const r = regionsNewCases.map(region => {
...
const Map = ({newCases}) => {
  ...
  useEffect(() => {
    simulateMinMaxNewCasesFetch(1000).then(result => {
  ...
  useEffect(() => {
    setRegionsNewCases(computeOpacities(newCases, minNewCases, maxNewCases));
  }, [newCases, minNewCases, maxNewCases]);
